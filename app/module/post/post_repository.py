...
class PostRepository(GenericRepository):
    ...
    async def get_recent_posts_by_mission(
    ...
    ) -> list[tuple[User, list[PostImage]]]:
        stmt = (
            select(Post.id, Post.created_at, User, PostImage)
        ...
        posts_dict: dict[int, tuple[User, list[PostImage]]] = {}
        for post_id, created_at, user, post_image in rows:
            if post_id not in posts_dict:
                posts_dict[post_id] = (user, [])

            if post_image:
                posts_dict[post_id][1].append(post_image)
